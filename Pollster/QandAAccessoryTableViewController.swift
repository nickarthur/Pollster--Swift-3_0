...
class QandAAccessoryTableViewController: QandATableViewController
{
    ...
    var accessoryValues = [String:CustomStringConvertible]() {
        didSet {
            for content in data?[Section.Answers] ?? [] {
                accessoryLabelFor(content: content)?.text = "\(accessoryValues[content] ?? "")"
            }
        }
    }
    ...
    private func accessoryLabelFor(content: String) -> UILabel? {
    ...
    override func createTextViewFor(indexPath: IndexPath?) -> UITextView {
        let textView = super.createTextViewFor(indexPath: indexPath)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if asking && indexPath.section == Section.Answers {
            if let answerInRow = data?[indexPath.section][indexPath.row] {
                accessoryLabelFor(content: answerInRow)?.text = "\(accessoryValues[answerInRow] ?? "")"
            }
        }
        return cell
    }
